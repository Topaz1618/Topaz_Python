...
class ExpressionProcess(object):
    ...
    def get_avg(self,data_list):
        # print("avg",len(data_list),data_list)
        clean_data_list = []
        for data in data_list:
            # print('1看看表达式了',self.expression_obj,type(self.expression_obj))
            # print('data',len(data_list),data)
            val,time = data
    ...
    def get_max(self,data_list):
        # print("max")
        for data in data_list:
            # print('2看看表达式了', self.expression_obj)
            # print('2瞅一眼', data)
            val,save_time = data
        else:#可能是由于最近这个服务 没有数据 汇报 过来,取到的数据 为空,所以没办法 判断阈值
            return [False,None,None]
